<xsl:stylesheet version="1.0"  xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
 ...
 <!-- XSLT stylesheet to convert the output of the 'tree' utility (or equivalent)
  ...
  <xsl:comment>Islandora audio structure file used by the audio Batch module. On batch ingest,
    ...
    audio object children. Files in directories named in child elements' 'content' attribute will be
    added as their datastreams. If 'islandora_compound_object' elements do not contain a MODS.xml file,
    the value of the 'title' attribute will be used as the parent's title/label.</xsl:comment>
  ...
  <!-- We aren't interested in these nodes, so apply an empty template to them. -->
    ...
    <!--xsl:choose-->
      <!--xsl:when test="count(file) > 1"-->
        <child content="{./@name}"/>
          <xsl:apply-templates/>
      <!--/xsl:when-->
      <!-- We also aren't interested in parent (of parent) wrappers -->
        <!--xsl:otherwise-->
        <!--parent title="{./@name}"-->
          <!--xsl:apply-templates/-->
        <!--/parent-->
